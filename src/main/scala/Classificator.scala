...
//    val tfidfIgnore: RDD[linalg.Vector] = idfIgnore.transform(tf)
    ...
    tfidf.foreach(v => println(v.toSparse.values.foreach(d => print(d))))

//



//    val d = tfidf.map(line => LabeledPoint(line.toSparse.values, [line.toArray.take(1)] )) // arbitrary mapping, it's just an example
